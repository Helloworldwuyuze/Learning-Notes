...
//	cout<<ls.fi<<" "<<ls.se<<" "<<rs.fi<<" "<<rs.se<<" "<<lcm<<" "<<d<<" "<<x<<endl;
...
signed main(){
    ...
    cin>>n;
//    build(1,1,n);
    for(int i=1;i<=n;++i)   cin>>b>>a, update(1,1,n,i,mk(a,b));
    cin>>m;
    while(m--){
        cin>>op;
        if(op==1){
...
//        cout<<endl<<"------------"<<endl;
//        for(int i=1;i<=n<<1;++i)	cout<<i<<" "<<t[i].fi<<" "<<t[i].se<<endl;
    }
    return 0;
}
/*
